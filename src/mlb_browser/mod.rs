...
impl MenuItem {
    pub fn render(&self, transform: Matrix2d, c: graphics::context::Context, gl: &mut GlGraphics) {
        ...
        let vs_text_width: f64 = vs_text
            .chars()
            .map(|c| glyph_cache.character(vs_text_font_size, c).unwrap().advance_width())
            .sum();
        let desc_text_width: f64 = desc_text
            .chars()
            .map(|c| glyph_cache.character(desc_text_font_size, c).unwrap().advance_width())
            .sum();
        ...
        if self.is_selected {
            ...
            let vs_trans = transform.trans(-vs_text_width / 2.0, -center_y * scale - vs_text_font_size as f64/2.0);
            let desc_trans = transform.trans(-desc_text_width / 2.0, center_y * scale + desc_text_font_size as f64);
            let img_trans = transform
                .scale(scaled_width / img_width, scaled_height / img_height)
                .trans(-0.5 * img_width, -0.5 * img_height);
            text(WHITE, vs_text_font_size, &vs_text, &mut glyph_cache, vs_trans, gl).unwrap();
            text(WHITE, desc_text_font_size, &desc_text, &mut glyph_cache, desc_trans, gl).unwrap();
